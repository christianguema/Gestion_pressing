...
$(document).on('click', '.delete-btn', function () {
  var id = $(this).data("id");
  $('#deletePersonnelForm').attr('action', '/personnels/' + id);
});

// Remplissage du modal personnel
$(document).on('click', '.view-btn', function () {
    const photoPath = $(this).attr('data-image');

    // $('#personnel-image').attr('src', $(this).data('image'));
    $('#personnel-nom').text(($(this).data('nom')) + ' ' + ($(this).data('prenom')));
    $('#personnel-poste').text($(this).attr('data-poste'));
    $('#personnels-poste').text($(this).attr('data-poste'));
    ...
    $('#pressing-adresse').text($(this).data('pressing_adresse'));


    // 🔁 Gérer l'image : si un chemin est fourni, on l'utilise, sinon on garde le default
    const defaultImage = "{{ asset('assets/img/default-avatar.png') }}";

    if (photoPath) {
        
        $('#personnel-image').attr('src', "{{ asset('storage') }}" + '/' + photoPath);
    } else {
        $('#personnel-image').attr('src', defaultImage);
    }
});
...
$('#client_search').on('input', function() {
    ...
    if (search && matches.length) {
        matches.forEach((c) => {
            $suggestions.append(
                `<button type="button" class="list-group-item list-group-item-action client-suggestion" data-id="${c.client_id}" data-name="${c.user.name}" data-lastname="${c.user.lastname}">${c.user.name} ${c.user.lastname}</button>`
            );
        });
        $suggestions.show();
