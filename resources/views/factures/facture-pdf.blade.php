...
@if(str_contains(strtolower($commande->typeFacturation->libelle ?? ''), 'kilo'))
    ...
    \begin{flushright}
        \begin{tabular}{lr}
            ...
            \hline
            \textbf{Montant Total :} & @latex(number_format($commande->prix_unitaire_kilo * $commande->poids_total, 0, ',', ' ')) FCFA \\
            @if($montant_remise > 0)
                \textbf{Remise :} & @latex(number_format($montant_remise, 0, ',', ' ')) FCFA \\
            @endif
            \hline
            \textbf{\color{lightblue}Net à Payer :} & \textbf{@latex(number_format(($commande->prix_unitaire_kilo * $commande->poids_total) - $montant_remise, 0, ',', ' ')) FCFA}
        \end{tabular}
    \end{flushright}
...
@else
    ...
    \begin{flushright}
        \begin{tabular}{lr}
            ...
            @endif
            \hline
            \textbf{\color{lightblue}Net à Payer :} & \textbf{@latex(number_format($commande->montant_total - $montant_remise, 0, ',', ' ')) FCFA}
        \end{tabular}
    \end{flushright}
...
\begin{minipage}{0.45\textwidth}
    \framebox[1.1\width]{\begin{minipage}{0.9\textwidth}\centering\vspace{2cm}\textbf{Signature Client}\vspace{0.5cm}\end{minipage}}
\end{minipage}
\hfill
\begin{minipage}{0.45\textwidth}
    \framebox[1.1\width]{\begin{minipage}{0.9\textwidth}\centering\vspace{2cm}\textbf{Cachet et Signature}\vspace{0.5cm}\end{minipage}}
\end{minipage}
