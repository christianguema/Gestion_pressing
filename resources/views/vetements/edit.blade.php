        ...
        <form action="{{ route('vetements.update', $vetement) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('type') is-invalid @enderror" id="type" name="type"
                    value="{{ old('type', $vetement->type) }}" required>
                @error('type')
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('prix_unitaire') is-invalid @enderror"
                    ...
                    value="{{ old('prix_unitaire', $vetement->prix_unitaire) }}">
                @error('prix_unitaire')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="categorie_id" id="categorie_id"
                    ...
                    <option value="{{ $cat->categorie_id }}" {{ $vetement->categorie_id == $cat->categorie_id ? 'selected' : '' }}>
                        {{ $cat->intitule }}
                    </option>
                    @endforeach
                </select>
