...
<div class="mb-3 d-flex justify-content-between align-items-center">
    ...
    <div>
        ...
        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton2">
            <li>
                <a class="dropdown-item {{ empty($pressingId) ? 'active' : '' }}" href="{{ route('personnel.index') }}">
                    Tous
                </a>
            </li>
            @foreach($pressings as $pressing)
                <li>
                    <a class="dropdown-item {{ (isset($pressingId) && $pressingId == $pressing->id) ? 'active' : '' }}"
                        href="{{ route('personnel.index', ['pressing_id' => $pressing->pressing_id]) }}">
                        {{ $pressing->nom }}
                    </a>
                </li>
            @endforeach
    ...
    <tr>
        ...
        <td>
            <button class="btn view-btn btn-info btn-sm"
                ...
                data-prenom="{{ $personnel->user->last_name }}"
                data-contact="{{ $personnel->user->contact }}"
                data-image="{{ asset('assets/img/profile-img.jpg') }}"
